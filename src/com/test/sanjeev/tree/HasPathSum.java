...
public class HasPathSum {
	...
	static Boolean helper(TreeNode node, int target, int sumSofar, Boolean hasPS) {
		...
		if(!hasPS &&node.left!=null)
			hasPS = helper(node.left, target,  sumSofar+node.val, hasPS);
		if(!hasPS &&node.right!=null)
			hasPS = helper(node.right, target,  sumSofar+node.val, hasPS);
		return hasPS;
	}
